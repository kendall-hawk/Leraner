    ...
    function AppController(config) {
        ...
        function setupModuleIntegration(options) {
            try {
                ...
                // 检查基础依赖
                if (!foundation) {
                    throw new Error('Foundation layer not initialized');
                }
                ...
                console.error('[AppController] Module integration setup failed:', error);
                // 即使失败也返回true，让应用继续启动
                return true;
            }
        }
            ...
            try {
                ...
                foundation.EventHub.on('glossary:bookmarkChanged', function(data) {
                    ...
                    if (foundation.CacheManager &&
                        typeof foundation.CacheManager.cache === 'function') {
                        try {
